...
DEFAULT_CATALOG_ROOTS = [Path.home() / ".config" / "skillshare" / "skills"]
...
def source_label(
    ...
    if resolved in import_roots:
        return "import"
    mapping = {
        (Path.home() / ".codex" / "skills").resolve(): "codex-target",
        (Path.home() / ".claude" / "skills").resolve(): "claude-target",
        (Path.home() / ".agents" / "skills").resolve(): "agents",
        (Path.home() / ".codex" / "skills" / ".system").resolve(): "codex-system",
    }
...
def parse_catalog(catalog_root: Path) -> tuple[list[CatalogEntry], list[Category]]:
    ...
    for raw_line in text.splitlines():
        ...
        if current_section == "分类体系" and len(cells) >= 3:
            ...
            category_id = Path(category_id).name
            if category_id == "分类目录":
                continue
        ...
        if current_section == "Catalog" and len(cells) >= 4:
            skill_name = clean_markdown_cell(cells[0])
            if skill_name.lower() == "skill":
                continue
            entries[normalize_name(skill_name)] = CatalogEntry(
                ...
                zh_name=clean_markdown_cell(cells[1]),
                trigger=clean_markdown_cell(cells[2]),
                use_when=clean_markdown_cell(cells[2]),
                do_not_use_when="",
            )

        if current_section == "Skill Registry" and len(cells) >= 4:
            skill_name = clean_markdown_cell(cells[0])
            if skill_name.lower() == "skill":
                continue
            entries[normalize_name(skill_name)] = CatalogEntry(
