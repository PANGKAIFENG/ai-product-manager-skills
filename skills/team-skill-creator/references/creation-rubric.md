...
- The GitHub catalog files in that checkout, including `SKILL_REGISTRY.md`, `README.md`, and same-name Skill directories.
- The target local `multica-skill` directory when the Skill should appear inside Multica.
- `$SKILLSHARE_SKILLS_ROOT`
- `$CODEX_HOME/skills` or the active Codex user Skill directory
- `$CLAUDE_SKILLS_ROOT` or the active Claude Code user Skill directory
- the installed system Skill directory
- the active Agent Skills directory
...
- Visibility: public, project-level, local restricted, or local draft. Keep sensitive content out of the public catalog until it is de-identified or split.
- Public source: `PANGKAIFENG/ai-product-manager-skills` for general, shareable Skills that pass disclosure and license checks.
...
- Status: `core`, `active`, or `review` in the public catalog.
- Discoverability updates: `中文速查`, `README.md`, and `SKILL_REGISTRY.md`.
...
- Use `skillshare target list --json` to discover local runtime targets instead of assuming Codex and Claude are the full set.
- When only a subset is allowed, use per-Skill `metadata.targets` and verify `skillshare sync --dry-run --json`; stop if the installed version cannot enforce the requested scope.
- Never run `skillshare push` from a mixed public/private aggregation directory. Publish from the selected authoritative Git checkout with an explicit pathspec.
- Treat Multica Skill content publication separately from project, Agent, and squad binding; require evidence for both when both are requested.
