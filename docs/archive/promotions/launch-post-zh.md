...
- 一个 Roadmap、定价或定位问题很复杂时，怎么先定题、规划探索路径，并在结束后留下可复用方法？
- 一个主题需要研究时，怎么把资料转成 PM 判断，而不是百科摘要？
- 一个 PRD 初稿出来后，怎么从研发和测试视角找交付阻断项？
- 一个 PRD 进入 UI 前，怎么先确认结构、状态和布局，而不是直接画高保真？
...
它是一个中文优先的 AI Product Manager Skills Library，目前包含 13 个公开 Skill：

- `ai-collaboration-calibration`：问题还没想清楚时，先挑战假设和目标。
- `complex-exploration`：复杂任务先定题、探索、收敛，并沉淀五类可复用资产。
- `research-topic-compiler`：系统研究主题、概念源流和行业演进，转成 PM 决策输入。
- `competitive-analysis`：把竞品、替代方案和可选产品走查证据转成产品决策简报。
...
- `ui-wireframe-to-html`：基于 PRD 先输出 UI 结构、状态模型和 ASCII 布局。
...
产品问题校准 -> 复杂探索资产化 -> 主题研究 -> 竞品决策分析 -> 决策调研 -> 方案脑暴 -> PRD 起草 -> PRD 评审 -> PRD 拆 issue -> UI 结构线框 -> 高保真 UI handoff / 方案压测 -> 交给 Superpowers 做开发计划。
