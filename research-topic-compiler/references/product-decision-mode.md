...
description: 产品候选研究模式——围绕产品策略选择的多源竞品研究、候选池和决策交接输入
---

# Product Candidate Research Mode
...
| | Normal Research | Learning Pack | Product Candidate |
|---|---|---|---|
| 目标 | 建立对主题的认知 | 系统学习陌生领域 | 形成候选池和决策输入 |
| 核心产物 | 证据矩阵 + 阶段结论 | 学习框架 + 实践任务 | Candidate Backlog + 评分 + 排除项 + handoff |
| 停止标准 | 问题覆盖充分 | 认知框架建立 | 候选池可评审 / 可转交 `decision-research` |
...
```markdown
**Product Candidate Research Result**
...
- Top candidates: [排序后 Top 5-15，作为决策输入]
- Excluded: [排除项 + 理由]
- Recommended exit: [`decision-research` / Top 3 starter / demo / PRD input]
...
## 何时不用 Product Candidate Mode

- 用户只是想了解一个主题（用 Normal Research）
- 用户要系统学习一个陌生领域（用 Learning Pack）
- 用户已经需要最终推荐和排除理由（用 `decision-research`）
