...
| `product-prd` | 产品与 PRD | 需求结构化、PRD 起草、PRD 图示、PRD 评审、issue backlog、UI mockup 和交付准备。 |
...
| `research-topic-compiler` | 专题研究编译器 / 概念源流研究助手 | “系统研究这个主题”“整理到 Obsidian”“概念解读”“概念源流”“PM 技术评审提问脚本”“行业演进看板” | 多渠道主题研究、系统学习、轻量概念解构、概念源流、语义演化、PM 决策看板和研究报告；需要长期更新时可启用 Research Radar Loop。 | 创建或评审 Skill；普通即时搜索；一次性摘要。 | core |
...
| `ui-mockup-desktop-workbench` | 桌面工作台 UI Mockup 生成器 | “基于 PRD 和 UI 规范出桌面端 mockup”“生成桌面工作台真实页面”“把 Agent Client PRD 做成 HTML mockup” | 已有 PRD、桌面工作台需求、UI 规范或参考竞品，需要输出可打开、可截图、可交付讨论的真实桌面页面 mockup，并覆盖关键状态。 | 问题还没定义清楚；只写 PRD；移动端或营销页；直接实现生产代码。 | active |
...
当多个 Skill 都可能被中文关键词触发时，先查 [SKILL_ROUTING.md](SKILL_ROUTING.md)。默认按用户当前阶段分流：问题未定义用 `ai-collaboration-calibration`，主题/概念学习用 `research-topic-compiler`，单次决策调研用 `decision-research`，PRD 起草用 `prd-architect`，PRD 评审用 `prd-review`，PRD 已 ready 要拆 GitHub implementation issues 用 `prd-to-issues`，桌面端真实页面 mockup 用 `ui-mockup-desktop-workbench`，已有方案压测用 `grill-me`，重复 AI 工作资产化判断用 `ai-work-assetization-diagnoser`。
