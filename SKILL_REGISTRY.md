...
当多个 Skill 都可能被中文关键词触发时，先查 [SKILL_ROUTING.md](SKILL_ROUTING.md)。默认按用户当前阶段分流：问题未定义用 `ai-collaboration-calibration`，主题/概念学习用 `research-topic-compiler`，单次技术决策用 `tech-research`，PRD 起草用 `prd-architect`，PRD 评审用 `prd-review`，已有方案压测用 `grill-me`。
